// Hierarchical logistic regression model, unadjusted
...
parameters {
  ...
  // location-level effects
    vector[N_loc] B0_loc; // Coefficients for location-level intercepts
  ...
  // household-level effects
    vector[N_hh] B0_hh; // Coefficients for household-level intercepts
    real<lower=0, upper=10> sigma0_hh; // SD of household-level intercepts
    
  // participant-level effects
    vector[N_part] B0_part; // Coefficients for participant-level intercepts
...
transformed parameters {
  ...
  for(h in 1:N_hh) {
    eff_hh[h] = eff_loc[loc_lookup[h]] + B0_hh[h];
  }
...
model {
  ...
  for(i in 1:N_tri) {
    ...
    log_odds[i] = eff_part[part_lookup[i]] + B0_rd[rd_lookup[i]];
